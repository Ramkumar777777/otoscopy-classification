...
# Load the model
num_classes = 9  # number of classes in your trained model
...
class_names = ['aom', 'csom', 'earVentilationTube', 'earwax', 'foreignObjectEar', 'normal', 'otitisexterna', 'pseudoMembranes', 'tympanoskleros']

# AI suggestion helper
def get_ai_suggestion(diagnosis: str) -> str:
    prompt = (
        f"The detected ear condition is '{diagnosis}'. "
        "Provide a brief, layperson-friendly explanation of what this condition means, "
        "common next steps, and general care tips (non-medical disclaimer)."
    )
    try:
        response = gi_client.chat.completions.create(
            model="gpt-4o-mini",
        ...
        app.logger.error(f"AI suggestion failed: {e}")
...
def predict_route():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            suggestion = get_ai_suggestion(predicted_class)
            # Save prediction
            record = Prediction(
